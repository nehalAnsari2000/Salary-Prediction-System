...
def main():
    ...
    try:
        ...
        X=dataset.iloc[:,0:1].values     #fn to fetch data column wise or row wise
        Y=dataset.iloc[:,-1].values

        s=float(input("\nEnter test data size (Between 0 and 1) : "))     # i.e.  0.1 for 10%
        X_train,X_test,Y_train,Y_test = train_test_split(X,Y,test_size=s, random_state=42)  # It will return 4 values 
        ...
        i=0
        print(f'{"X_test":>15}{"Y_test":>20}{"Y_pred":>20}')
        while i<len(X_test):
            print(f'{X_test[i][0]:>15}{Y_test[i]:>20}{Y_pred[i]:>20.2f}')
            i+=1
        ...
        rounded_d = d.round({'Salaries':2})
        print("\n Here is the result \n")
        print(rounded_d.to_string(index=False))
        print()
